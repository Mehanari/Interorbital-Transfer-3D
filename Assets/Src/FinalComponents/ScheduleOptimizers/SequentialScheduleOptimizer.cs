...
{
    public class SequentialScheduleOptimizer
    {
        ...
        private readonly KinematicCalculator _kinematicCalculator;
        ...
        public (double[] driftTimes, double[] transferTimes) OptimizeSchedule(double driftTimeInit, double transferTimeInit,
            TargetParameters[] targets, Orbit spacecraftInitialOrbit, double spacecraftFinalMass)
        {
            ...
            for (int i = 0; i < targets.Length; i++)
            {
                ...
                var (drift, transfer, nextOrbit) = OptimizeForOne(driftTimeInit, transferTimeInit, elapsedTime,
            ...
            TargetParameters target, Orbit spacecraftCurrentOrbit)
        {
            ...
            var min = GradientDescent.Minimize(Objective, initialGuess, GdStepSize, GdTolerance, GdIterationsLimit,
                projection: Project);
            var kinematic =
                _kinematicCalculator.CalculateKinematics(min[0], min[1], elapsedTime, target, spacecraftCurrentOrbit);
            ...
            double Objective(Vector times)
            {
                ...
                var kinematics = _kinematicCalculator.CalculateKinematics(driftTime, transferTime, elapsedTime, target,
                    spacecraftCurrentOrbit);
                var cost = _costCalculator.CalculateCost(new KinematicData[] { kinematics }, spacecraftFinalMass);
